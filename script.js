...
// Start background music (calm, low volume)
bgMusic.volume = 0.15;
bgMusic.play();
...
function checkMilestone() {
  // Trigger multipliers & effects at every 100k points
  const milestoneStep = 100000;
  if (score >= lastMilestone + milestoneStep) {
    multiplier++;
...
function gameLoop() {
  if (holding) {
    ...
    const pickleScreenPos = Math.max(20, gameHeight / 2 - 40); // never below bottom
